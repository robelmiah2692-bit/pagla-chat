...
class SoulmateAnimationService {
  ...
  static Widget buildSoulmateHeartOverlay({
    ...
    required String myPartnerAuthUID,  // ফায়ারস্টোর প্রোফাইল থেকে আসা সোলমেট আইডি
    required String myCurrentAuthUID,  // ইউজারের নিজের লম্বা 'authUID'
  }) {
    if (seats.isEmpty || myPartnerAuthUID.isEmpty) return const SizedBox.shrink();

    int mySeatIndex = -1;
    int partnerSeatIndex = -1;

    // ১. সিট লিস্ট লুপ করে আইডি মিলিয়ে ইনডেক্স বের করা
    for (int i = 0; i < seats.length; i++) {
      var seat = seats[i];
      if (seat == null) continue;
      
      String? seatAuthUID = seat["authUID"]?.toString() ?? seat["userId"]?.toString();
      String? seatUID = seat["uID"]?.toString(); // ৬ ডিজিটের আইডি

      // নিজের আইডি চেক
      if (seatAuthUID == myCurrentAuthUID || (seatUID != null && seatUID == myCurrentAuthUID)) {
        mySeatIndex = i; 
      }
      
      // পার্টনারের আইডি চেক
      if (seatAuthUID == myPartnerAuthUID || (seatUID != null && seatUID == myPartnerAuthUID)) {
        partnerSeatIndex = i; 
      }
    }
    ...
    if (isAdjacent && isSameRow) {
      ...
      int leftSeat = (mySeatIndex < partnerSeatIndex) ? mySeatIndex : partnerSeatIndex;
      ...
      return Builder(
        builder: (context) {
          ...
          return Positioned(
            ...
            child: IgnorePointer(
              child: SizedBox(
                width: 80,  // 🔥 ফিক্স: বাইরের কন্টেইনার সাইজ ৮০ করা হলো যেন লটি বড় হতে পারে
                height: 80, // 🔥 ফিক্স: বাইরের কন্টেইনার সাইজ ৮০ করা হলো যেন লটি বড় হতে পারে
                child: Stack(
